...
function openKPIDetail(kpiId){
  ...
  openModal('wide', `
    ...
    <div class="modal-body">
      ...
      ${canApprove ? `<div style="margin-bottom:16px">
        <button class="btn btn-sm" style="background:${k.approvalStatus==='Approved'?'var(--green)':'#fff'};color:${k.approvalStatus==='Approved'?'#fff':'var(--green)'};border:1.5px solid var(--green)" onclick="approveKPI('${k.id}','Approved')">${k.approvalStatus==='Approved'?'✓ Approved':'Approve'}</button>
        <button class="btn btn-sm" style="background:${k.approvalStatus==='Rejected'?'var(--red)':'#fff'};color:${k.approvalStatus==='Rejected'?'#fff':'var(--red)'};border:1.5px solid var(--red);margin-left:6px" onclick="approveKPI('${k.id}','Rejected')">${k.approvalStatus==='Rejected'?'✕ Rejected':'Reject'}</button>
        ${(k.approvalStatus&&k.approvalStatus!=='Pending')?`<button class="btn btn-sm btn-outline" style="margin-left:6px" onclick="approveKPI('${k.id}','Pending')">Reset</button>`:''}
