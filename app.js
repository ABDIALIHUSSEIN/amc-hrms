...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (saved && saved.user && saved.user.name && saved.rememberMe) {
    // Valid session — restore and skip login screen
    STATE.user       = saved.user;
    STATE.role       = saved.role       || 'viewer';
    STATE.subsidiary = saved.subsidiary || 'all';
    ...
    if (saved && saved.user && saved.user.email) {
      ...
      if (emailField) emailField.value = saved.user.email;
    }
  }
...
function updateNavBadges() {
  ...
  } catch(e) { console.warn('Badge update failed:', e.message); }
...
function exportPayroll() {
  const rows = DB.payroll.map(p => {
    ...
    return { 'ID': e.id, 'Name': e.name, 'Subsidiary': getSubName(e.sub), 'Department': getDeptName(e.dept),
      'Base Salary': c.baseSalary, 'Allowance': c.allowance, 'OT Pay': c.otPay, 'Eid Bonus': c.eidBonus,
      'Gross': c.grossEarnings, 'Tax': c.tax, 'Advance': c.advanceDeduct, 'Deductions': c.totalDeductions,
      'Net Pay': c.netPay, 'Gratuity': c.gratuity, 'Status': c.status, 'Month': p.month };
  }).filter(Boolean);
...
function viewEmpModal(id) {
  ...
  const lb = DB.leaveBalances[id]||{annual:21,sick:14,maternity:0,paternity:0,used_annual:0,used_sick:0,used_maternity:0};
...
function openCreateUserModal() {
  ...
  openModal('narrow', `
    <div class="modal-header" style="border-bottom:1px solid var(--gray-100);padding:20px 24px 16px"> <div> <div class="modal-title" style="font-size:18px;font-weight:800;color:var(--gray-900)">Create User Account</div> <div style="font-size:12px;color:var(--gray-400);margin-top:3px">Add a new system user and assign their role</div> </div> ${closeX()}
        ...
        </select> </div> <!-- Link Employee --> <div class="form-group" style="margin-bottom:14px"> <label class="form-label" style="font-size:12px;font-weight:700;color:var(--gray-600);text-transform:uppercase;letter-spacing:.4px">Link to Employee</label> <select class="form-control" id="cu_emp"
...
function leaveBalancesHTML(){
  return `<div class="card"><div class="table-wrap"><table class="table"> <thead><tr><th>Employee</th><th>Subsidiary</th><th>Annual (Rem/Total)</th><th>Sick</th><th>Maternity</th><th>Used Annual</th><th></th></tr></thead> <tbody>${filteredEmps().slice(0,30).map(e=>{
      const lb=DB.leaveBalances[e.id]||{annual:21,sick:14,maternity:0,used_annual:0,used_sick:0};
...
PAGES.payroll = function(wrap) {
  // All active employees, merge with existing payroll record for selected month
  const rows = filteredEmps().map(e => {
    const p = DB.payroll.find(x => x.empId === e.id && x.month === STATE.payMonth)
           || { empId: e.id, month: STATE.payMonth, baseSalary: e.salary, allowance: e.allowance,
                otHours: 0, advance: 0, lateDeduction: 0, absentDeduction: 0, eidBonus: 0, status: 'Pending' };
    return { ...p, ...PayrollEngine.calc(e, p), empName: e.name, empSub: getSubName(e.sub), empDept: getDeptName(e.dept) };
  });

  const sg=rows.reduce((s,r)=>s+r.grossEarnings,0),sn=rows.reduce((s,r)=>s+r.netPay,0),
        sd=rows.reduce((s,r)=>s+r.totalDeductions,0),st=rows.reduce((s,r)=>s+r.tax,0);

  wrap.innerHTML=`<div class="page"> <div class="page-header"> <div class="page-header-left"><div class="page-title">Payroll Management</div> <div class="page-sub">Base · Allowances · OT (×1.5) · Tax · Eid Bonus · Gratuity · Advances max 50%</div></div> <div class="page-actions"> <input type="month" class="form-control" style="width:auto" value="${STATE.payMonth}" onchange="STATE.payMonth=this.value;nav('payroll')"> <button class="btn btn-outline btn-sm" onclick="exportPayroll()">${ICO.excel} Export</button> <button class="btn btn-primary btn-sm" onclick="runPayrollBatch()">${ICO.play} Run Batch</button> </div> </div> <div class="stat-grid"> <div class="stat-card gold"><div class="stat-info"><div class="stat-label">Gross Payroll</div><div class="stat-val">${fmtCurrency(sg).split('.')[0]}</div></div></div> <div class="stat-card navy"><div class="stat-info"><div class="stat-label">Net Payroll</div><div class="stat-val">${fmtCurrency(sn).split('.')[0]}</div></div></div> <div class="stat-card red"><div class="stat-info"><div class="stat-label">Total Deductions</div><div class="stat-val">${fmtCurrency(sd).split('.')[0]}</div></div></div> <div class="stat-card purple"><div class="stat-info"><div class="stat-label">Total Tax</div><div class="stat-val">${fmtCurrency(st).split('.')[0]}</div></div></div> <div class="stat-card green"><div class="stat-info"><div class="stat-label">Processed</div><div class="stat-val">${rows.filter(r=>r.status==='Processed').length}/${rows.length}</div></div></div> <div class="stat-card amber"><div class="stat-info"><div class="stat-label">Pending</div><div class="stat-val">${rows.filter(r=>r.status==='Pending').length}</div></div></div> </div> <div class="card"> <div class="card-header" style="flex-wrap:wrap;gap:10px"> <div class="card-title">Payroll Register — ${STATE.payMonth}</div> <input id="paySearch" type="text" class="form-control" style="width:220px;margin-left:auto" placeholder="Search employee…"> </div> <div style="font-size:12px;color:var(--gray-500);padding:0 20px 10px">OT=(Base÷22÷8)×1.5 · Tax: 4%(≤$3k) 6%(≤$6k) 8%(>$6k) · Advance max 50%</div> <div class="card-body" style="padding-top:0"><div class="table-wrap"><table class="table"> <thead><tr><th>Employee</th><th>Subsidiary</th><th>Base</th><th>Allow.</th><th>OT Pay</th><th>Gross</th><th>Tax</th><th>Advance</th><th>Net Pay</th><th>Gratuity</th><th>Status</th><th>Actions</th></tr></thead> <tbody id="payTbody">${rows.map(r=>`<tr data-search="${(r.empName+' '+r.empId).toLowerCase()}"> <td><div class="emp-cell"><div class="avatar-sm" style="width:28px;height:28px;font-size:10px">${initials(r.empName)}</div><div><div class="emp-name">${r.empName}</div><div class="emp-id">${r.empId}</div></div></div></td> <td style="font-size:11px">${r.empSub}</td> <td style="font-family:var(--mono);font-size:12px">${fmtCurrency(r.baseSalary)}</td> <td style="font-family:var(--mono);font-size:12px">${fmtCurrency(r.allowance)}</td> <td style="font-family:var(--mono);font-size:12px;color:${r.otPay>0?'var(--teal)':'var(--gray-300)'}">${r.otPay>0?fmtCurrency(r.otPay):'—'}</td> <td style="font-family:var(--mono);font-weight:700;font-size:12px">${fmtCurrency(r.grossEarnings)}</td> <td style="font-family:var(--mono);font-size:12px;color:var(--red)">-${fmtCurrency(r.tax)}</td> <td style="font-family:var(--mono);font-size:12px;color:${r.advanceDeduct>0?'var(--amber)':'var(--gray-300)'}">${r.advanceDeduct>0?`-${fmtCurrency(r.advanceDeduct)}`:'—'}</td> <td style="font-family:var(--mono);font-weight:800;color:var(--navy)">${fmtCurrency(r.netPay)}</td> <td style="font-family:var(--mono);font-size:12px;color:var(--green)">${fmtCurrency(r.gratuity)}</td> <td><span class="badge ${r.status==='Processed'?'badge-green':'badge-amber'}">${r.status}</span></td> <td><button class="btn btn-outline btn-xs" onclick="genPayslipModal('${r.empId}')">${ICO.eye}</button></td> </tr>`).join('')}</tbody> </table></div></div> </div> </div>`;

  wrap.querySelector('#paySearch').addEventListener('input', function() {
    const q = this.value.toLowerCase().trim();
    wrap.querySelectorAll('#payTbody tr').forEach(tr => {
...
async function resetSupabase() {
  ...
  for (const t of tables) {
    try { await SUPA.delete(t, 'id=neq.00000000-0000-0000-0000-000000000000'); } catch(e) { console.warn(`Reset: could not clear table ${t}:`, e.message); }
  }
...
const SELF_PAGES = {
  ...
  dashboard(wrap) {
    ...
    const lb  = DB.leaveBalances[emp.id] || { annual:21, sick:14, used_annual:0, used_sick:0 };
...
function renderNoticeList() {
  ...
  wrap.innerHTML = notices.map(n => {
    ...
    return `<div class="card" style="${border};margin-bottom:12px;transition:box-shadow .2s" onmouseenter="this.style.boxShadow='0 4px 16px rgba(0,0,0,.08)'" onmouseleave="this.style.boxShadow=''"> <div style="padding:16px 20px"> <!-- Header row --> <div style="display:flex;align-items:flex-start;gap:12px;margin-bottom:10px;flex-wrap:wrap"> <div style="font-size:22px;flex-shrink:0">${noticeCatIcon(n.category)}</div> <div style="flex:1;min-width:0"> <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-bottom:4px"> ${n.pinned ? '<span style="font-size:11px;color:var(--amber);font-weight:800"> PINNED</span>' : ''}
              ...
              onclick="openNoticeDetail('${n.id}')">${n.title}</div> </div> ${isMgr ? `<div style="display:flex;gap:6px;flex-shrink:0"> <button class="btn btn-outline btn-sm" onclick="openEditNoticeModal('${n.id}')">Edit</button> <button class="btn btn-outline btn-sm" onclick="archiveNotice('${n.id}')" style="color:var(--gray-400)">${n.status==='Archived'?'Unarchive':'Archive'}</button> </div>` : ''}
...
function openNoticeDetail(noticeId) {
  ...
  openModal('wide', `
    <div style="border-top:4px solid ${priColor[n.priority]||'#6B7280'}"> <div class="modal-header" style="padding-top:16px"> <div style="flex:1"> <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;flex-wrap:wrap"> ${n.pinned ? '<span style="font-size:11px;color:var(--amber);font-weight:800"> PINNED</span>' : ''}
          ${noticePriBadge(n.priority)}
          <span style="font-size:12px;font-weight:700;color:var(--gray-500)">${noticeCatIcon(n.category)} ${n.category}</span> </div> <div style="font-size:18px;font-weight:900;color:var(--gray-900);line-height:1.3">${n.title}</div> <div style="font-size:12px;color:var(--gray-400);margin-top:4px"> Published by ${n.createdByName} · ${fmtDate(n.publishDate)}
          ${n.expiryDate ? ` · Expires ${fmtDate(n.expiryDate)}` : ''}
        </div> </div> ${closeX()}
    </div> <div class="modal-body"> <!-- Body --> <div style="font-size:14px;line-height:1.8;color:var(--gray-700);white-space:pre-wrap;background:var(--gray-50);border-radius:var(--radius);padding:16px 20px;margin-bottom:16px">${n.body}</div> <!-- Meta grid --> <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:16px"> ${[
...
function saveNotice(existingId, status) {
  ...
  const body    = document.getElementById('nc_body')?.value?.trim();
...
function showLoginNoticePopup() {
  ...
  openModal('wide', `
    <div style="border-top:4px solid #DC2626"> <div class="modal-header" style="padding-top:16px;background:linear-gradient(135deg,#7F1D1D,#DC2626)"> <div> <div style="font-size:11px;font-weight:800;color:rgba(255,255,255,.7);margin-bottom:4px">IMPORTANT NOTICE — ACTION REQUIRED</div> <div style="font-size:16px;font-weight:900;color:white">${n.title}</div> </div> ${closeX('white')}
...
function changeProfilePassword() {
  ...
  if (!DEMO_PASSWORDS.includes(curr)) { toast('Current password is incorrect', 'error'); return; }
  DB.auditLogs.unshift({ id:DB.auditLogs.length+1, time:new Date().toISOString().replace('T',' ').slice(0,16), user:STATE.user?.initials||'SYS', userRole:STATE.role, action:'Password changed via profile', module:'Profile', ip:'browser' });
  closeModal();
...
function saveEmployeeForm(existingId) {
  ...
  const empData = {
    ...
    address:         document.getElementById('ea_address')?.value || '',
    sub, dept, title,
...
function openMasterProfileModal(empId) {
  ...
  openModal('xl', `
    <div style="display:flex;flex-direction:column;height:85vh;overflow:hidden"> <!-- ── HEADER BANNER ── --> <div style="background:linear-gradient(135deg,var(--navy),#002D72);padding:20px 24px;flex-shrink:0"> <div style="display:flex;align-items:center;gap:16px;flex-wrap:wrap"> <div style="width:64px;height:64px;border-radius:50%;background:linear-gradient(135deg,var(--gold),#E8B84B);display:flex;align-items:center;justify-content:center;font-size:22px;font-weight:900;color:var(--navy);border:3px solid rgba(255,255,255,.2);flex-shrink:0"> ${initials(e.name)}
      ...
      </div> <!-- ── TAB CONTENT ── --> <div style="flex:1;overflow-y:auto;padding:0" id="mpBody"> <!-- ════ OVERVIEW ════ --> <div class="mp-pane" id="mp_overview" style="padding:20px 24px"> <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin-bottom:20px"> <div class="stat-card blue"><div class="stat-val">${fmtCurrency(e.salary||0)}</div><div class="stat-lbl">Monthly Salary</div></div> <div class="stat-card ${attLate>3?'amber':'green'}"><div class="stat-val">${attPresent}</div><div class="stat-lbl">Present (Recent)</div><div class="stat-sub">${attLate} late</div></div> <div class="stat-card ${(lb.annual||21)-(lb.used_annual||0)<5?'red':'teal'}"><div class="stat-val">${(lb.annual||21)-(lb.used_annual||0)}</div><div class="stat-lbl">Leave Remaining</div><div class="stat-sub">${lb.used_annual||0} used</div></div> <div class="stat-card ${sc!==null?(sc>=90?'green':sc>=70?'blue':'amber'):'navy'}"><div class="stat-val">${sc!==null?sc+'%':'—'}</div><div class="stat-lbl">KPI Score</div><div class="stat-sub">${rt?.label||'No data'}</div></div> </div> <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px"> <div class="card" style="margin:0"> <div class="card-header"><span class="card-title">Quick Info</span></div> <div class="card-body"> ${[['Email',e.email||'—'],['Phone',e.phone||'—'],['Join Date',fmtDate(e.joined)],['Department',getDeptName(e.dept)],['Subsidiary',getSubName(e.sub)],['Grade',e.grade||'—'],['Manager',e.managerName||'—'],['System Role',roleInfo?.label||'Not linked']].map(([k,v])=>`<div class="info-row"><span class="info-key">${k}</span><span class="info-val">${v}</span></div>`).join('')}
          ...
          <div class="card" style="margin:0"><div class="card-header"><span class="card-title">KPI Indicators</span></div> <div class="card-body" style="padding:0"><div class="table-wrap"><table class="table"> <thead><tr><th>KPI Title</th><th>Target</th><th>Actual</th><th>Weight</th><th>Achievement</th></tr></thead> <tbody>${kpis.length ? kpis.map(k=>{const ach=k.target>0?Math.round(k.actual/k.target*100):0;return`<tr> <td style="font-weight:600">${k.title}</td> <td style="font-family:var(--mono)">${k.target} ${k.unit||''}</td> <td style="font-family:var(--mono)">${k.actual} ${k.unit||''}</td> <td>${k.weight}%</td> <td><span class="kpi-score ${ach>=100?'excellent':ach>=70?'good':ach>=50?'average':'poor'}">${ach}%</span></td> </tr>`;}).join('') : '<tr><td colspan="5" style="text-align:center;color:var(--gray-400);padding:20px">No KPIs assigned</td></tr>'}</tbody> </table></div></div> </div> </div> <!-- ════ FINANCE (Loans & Advances) ════ --> <div class="mp-pane" id="mp_finance" style="display:none;padding:20px 24px"> <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:16px"> <div class="card" style="margin:0"><div class="card-header"><span class="card-title">Loans</span><button class="btn btn-outline btn-sm" onclick="closeModal();openCreateLoanModal()">+ Issue Loan</button></div> <div class="card-body" style="padding:0"> ${myLoans.length ? myLoans.map(l=>{const remain=Math.max(0,l.totalRepayable-l.amountPaid);const pct=Math.round(l.amountPaid/l.totalRepayable*100);return`
