...
PAGES.payroll = function(wrap) {
  ...
  wrap.innerHTML=`<div class="page"> <div class="page-header"> <div class="page-header-left"><div class="page-title">Payroll Management</div> <div class="page-sub">Base · Allowances · OT (×1.5) · No Tax · Eid Bonus · Gratuity · Advances max 50%</div></div> <div class="page-actions"> <input type="month" class="form-control" style="width:auto" value="${STATE.payMonth}" onchange="STATE.payMonth=this.value"> <button class="btn btn-outline btn-sm" onclick="exportPayroll()">${ICO.excel} Export</button> <button class="btn btn-primary btn-sm" onclick="runPayrollBatch()">${ICO.play} Run Batch</button> </div> </div> <div class="stat-grid"> <div class="stat-card gold"><div class="stat-info"><div class="stat-label">Gross Payroll</div><div class="stat-val">${fmtCurrency(sg).split('.')[0]}</div></div></div> <div class="stat-card navy"><div class="stat-info"><div class="stat-label">Net Payroll</div><div class="stat-val">${fmtCurrency(sn).split('.')[0]}</div></div></div> <div class="stat-card red"><div class="stat-info"><div class="stat-label">Total Deductions</div><div class="stat-val">${fmtCurrency(sd).split('.')[0]}</div></div></div> <div class="stat-card green"><div class="stat-info"><div class="stat-label">Processed</div><div class="stat-val">${rows.filter(r=>r.status==='Processed').length}/${rows.length}</div></div></div> <div class="stat-card amber"><div class="stat-info"><div class="stat-label">Pending</div><div class="stat-val">${rows.filter(r=>r.status==='Pending').length}</div></div></div> </div> <div class="card"> <div class="card-header"><div class="card-title">Payroll Register — ${STATE.payMonth}</div> <div style="font-size:12px;color:var(--gray-500)">OT=(Base÷22÷8)×1.5 · No income tax · Advance max 50%</div> </div> <div class="card-body"><div class="table-wrap"><table class="table"> <thead><tr><th>Employee</th><th>Subsidiary</th><th>Base</th><th>Allow.</th><th>OT Pay</th><th>Gross</th><th>Advance</th><th>Net Pay</th><th>Gratuity</th><th>Status</th><th>Actions</th></tr></thead> <tbody>${rows.map(r=>`<tr> <td><div class="emp-cell"><div class="avatar-sm" style="width:28px;height:28px;font-size:10px">${initials(r.empName)}</div><div><div class="emp-name">${r.empName}</div><div class="emp-id">${r.empId}</div></div></div></td> <td style="font-size:11px">${r.empSub}</td> <td style="font-family:var(--mono);font-size:12px">${fmtCurrency(r.baseSalary)}</td> <td style="font-family:var(--mono);font-size:12px">${fmtCurrency(r.allowance)}</td> <td style="font-family:var(--mono);font-size:12px;color:${r.otPay>0?'var(--teal)':'var(--gray-300)'}">${r.otPay>0?fmtCurrency(r.otPay):'—'}</td> <td style="font-family:var(--mono);font-weight:700;font-size:12px">${fmtCurrency(r.grossEarnings)}</td> <td style="font-family:var(--mono);font-size:12px;color:${r.advanceDeduct>0?'var(--amber)':'var(--gray-300)'}">${r.advanceDeduct>0?`-${fmtCurrency(r.advanceDeduct)}`:'—'}</td> <td style="font-family:var(--mono);font-weight:800;color:var(--navy)">${fmtCurrency(r.netPay)}</td> <td style="font-family:var(--mono);font-size:12px;color:var(--green)">${fmtCurrency(r.gratuity)}</td> <td><span class="badge ${r.status==='Processed'?'badge-green':'badge-amber'}">${r.status}</span></td> <td><div style="display:flex;gap:4px"><button class="btn btn-outline btn-xs" title="Edit pay inputs" onclick="openPayrollEditModal('${r.empId}')">${ICO.edit}</button><button class="btn btn-outline btn-xs" title="View payslip" onclick="genPayslipModal('${r.empId}')">${ICO.eye}</button></div></td> </tr>`).join('')}</tbody> </table></div></div> </div> </div>`;
};
...
function genPayslipModal(empId){
  ...
  openModal('wide',`
    <div class="modal-header"><span class="modal-title">Payslip — ${e.name} · ${STATE.payMonth}</span>${closeX()}</div> <div class="modal-body"> <div class="payslip-preview"> <div class="payslip-header"> <div><div class="payslip-company">Asal Media Corporation</div><div class="payslip-title">${getSubName(e.sub)}</div></div> <div class="payslip-period"><div style="font-weight:800;font-size:15px">PAYSLIP</div><div>${STATE.payMonth}</div></div> </div> <div class="payslip-emp"> <div class="avatar-lg">${initials(e.name)}</div> <div><div style="font-size:16px;font-weight:700">${e.name}</div> <div style="font-size:12px;color:var(--gray-500)">${e.title} · ${getDeptName(e.dept)}</div> <div style="font-size:11px;color:var(--gray-400);font-family:var(--mono)">${e.id} · Grade ${e.grade || '—'} · ${yos} yrs service</div> </div> </div> <table class="payslip-table" style="width:100%"> <tr><td colspan="2" style="font-weight:700;color:var(--gray-600);padding:10px 0 4px;text-transform:uppercase;font-size:10px;letter-spacing:.6px">EARNINGS</td></tr> <tr><td class="label">Basic Salary</td><td class="amount">${fmtCurrency(pc.baseSalary)}</td></tr> <tr><td class="label">Allowances</td><td class="amount">${fmtCurrency(pc.allowance)}</td></tr> ${pc.otPay>0?`<tr><td class="label">Overtime Pay (${p.otHours}h)</td><td class="amount" style="color:var(--teal)">${fmtCurrency(pc.otPay)}</td></tr>`:''}
          ...
          <tr style="border-top:2px solid var(--gray-200)"><td style="font-weight:700;padding-top:8px">Gross Earnings</td><td class="amount" style="font-weight:800;padding-top:8px">${fmtCurrency(pc.grossEarnings)}</td></tr> <tr><td colspan="2" style="font-weight:700;color:var(--gray-600);padding:10px 0 4px;text-transform:uppercase;font-size:10px;letter-spacing:.6px">DEDUCTIONS</td></tr> ${pc.advanceDeduct>0?`<tr><td class="label" style="color:var(--amber)">Salary Advance</td><td class="amount" style="color:var(--amber)">-${fmtCurrency(pc.advanceDeduct)}</td></tr>`:''}
...
PAGES.advances = function(wrap) {
  ...
  wrap.innerHTML = `<div class="page"> <div class="page-header"> <div><h1 class="page-title">Salary Advances</h1><div class="page-sub">${isMgr ? 'All employee advance requests' : 'My salary advance requests'}</div></div> <button class="btn btn-primary" onclick="openAdvanceRequestModal()">+ Request Advance</button> </div> <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin-bottom:20px"> <div class="stat-card amber"><div class="stat-val">${pending}</div><div class="stat-lbl">Pending Approval</div></div> <div class="stat-card green"><div class="stat-val">${fmtCurrency(approved)}</div><div class="stat-lbl">Approved (To Deduct)</div></div> <div class="stat-card blue"><div class="stat-val">${fmtCurrency(total)}</div><div class="stat-lbl">Total Issued YTD</div></div> <div class="stat-card navy"><div class="stat-val">${advances.length}</div><div class="stat-lbl">Total Requests</div></div> </div> <div class="card"><div class="table-wrap"><table class="table"> <thead><tr> ${isMgr ? '<th>Employee</th>' : ''}
        ...
        return `<tr> ${isMgr ? `<td><div style="font-weight:700">${emp?.name||'—'}</div><div style="font-size:11px;color:var(--gray-400)">${a.empId}</div></td>` : ''}
          <td style="font-family:var(--mono);font-weight:800;color:var(--navy)">${fmtCurrency(a.amount)}</td> <td style="font-size:12px">${a.reason}</td> <td style="font-size:11px;font-family:var(--mono)">${a.requestedAt}</td> <td>${finBadge(a.status)}</td> <td style="font-size:12px;font-family:var(--mono)">${a.deductMonth||'—'}</td> <td style="display:flex;gap:6px;flex-wrap:nowrap;align-items:center"> ${a.status==='Pending' && isMgr ? `
              <button style="width:34px;height:34px;flex-shrink:0;border-radius:10px;border:none;background:#D1FAE5;cursor:pointer;display:inline-flex;align-items:center;justify-content:center" title="Approve" onclick="approveAdvance('${a.id}')"><svg width='16' height='16' viewBox='0 0 24 24' fill='none' stroke='#065F46' stroke-width='2.5' stroke-linecap='round' stroke-linejoin='round'><polyline points='20 6 9 17 4 12'/></svg></button>
              <button style="width:34px;height:34px;flex-shrink:0;border-radius:10px;border:none;background:#FEE2E2;cursor:pointer;display:inline-flex;align-items:center;justify-content:center" title="Reject" onclick="rejectAdvance('${a.id}')"><svg width='14' height='14' viewBox='0 0 24 24' fill='none' stroke='#991B1B' stroke-width='2.5' stroke-linecap='round'><line x1='18' y1='6' x2='6' y2='18'/><line x1='6' y1='6' x2='18' y2='18'/></svg></button>` : ''}
          ${a.status==='Approved' && canApproveFinance() ? `
            <button style="width:34px;height:34px;flex-shrink:0;border-radius:10px;border:none;background:#DBEAFE;color:#1E40AF;cursor:pointer;display:inline-flex;align-items:center;justify-content:center" title="Mark as Paid" onclick="markAdvancePaid('${a.id}')">${ICO.dollar}</button>` : ''}
          ...
          ${['Approved','Rejected'].includes(a.status) && canApproveFinance() ? `<button style="width:34px;height:34px;flex-shrink:0;border-radius:10px;border:none;background:var(--gray-100);color:var(--gray-600);cursor:pointer;display:inline-flex;align-items:center;justify-content:center" title="Roll back to Pending" onclick="rollbackAdvance('${a.id}')">${ICO.refresh}</button>` : ''}
          </td> </tr>`;
...
PAGES.loans = function(wrap) {
  ...
  wrap.innerHTML = `<div class="page"> <div class="page-header"> <div><h1 class="page-title">Loan Management</h1><div class="page-sub">${isMgr ? 'All employee loans' : 'My loans'}</div></div> ${isMgr ? `<button class="btn btn-primary" onclick="openCreateLoanModal()">+ Issue Loan</button>` :
        ...
        return `<tr> ${isMgr ? `<td><div style="font-weight:700">${emp?.name||'—'}</div><div style="font-size:11px;color:var(--gray-400)">${l.empId}</div></td>` : ''}
          <td style="font-family:var(--mono);font-weight:700">${fmtCurrency(l.principal)}</td> <td style="font-family:var(--mono);color:var(--amber)">${fmtCurrency(l.monthlyInstallment)}</td> <td style="font-family:var(--mono);color:var(--red);font-weight:700">${fmtCurrency(remain)}</td> <td style="min-width:100px"> <div style="background:var(--gray-100);border-radius:99px;height:6px;overflow:hidden"> <div style="height:100%;background:${pct>=100?'var(--green)':'var(--blue)'};border-radius:99px;width:${Math.min(100,pct)}%"></div> </div> <div style="font-size:10px;color:var(--gray-400);margin-top:2px">${pct}% paid</div> </td> <td style="font-size:12px">${l.interestRate}%</td> <td style="font-size:12px">${l.months}mo</td> <td style="font-size:12px;max-width:140px">${l.purpose}</td> <td>${finBadge(l.status)}</td> <td style="display:flex;gap:6px;flex-wrap:nowrap;align-items:center"> <button style="width:34px;height:34px;flex-shrink:0;border-radius:10px;border:1px solid var(--gray-200);background:var(--white);cursor:pointer;display:inline-flex;align-items:center;justify-content:center;color:var(--gray-600)" title="Details" onclick="openLoanDetail('${l.id}')">${ICO.eye}</button> ${l.status==='Pending' && isMgr ? `
            <button style="width:34px;height:34px;flex-shrink:0;border-radius:10px;border:none;background:#D1FAE5;cursor:pointer;display:inline-flex;align-items:center;justify-content:center" title="Approve" onclick="approveLoan('${l.id}')"><svg width='16' height='16' viewBox='0 0 24 24' fill='none' stroke='#065F46' stroke-width='2.5' stroke-linecap='round' stroke-linejoin='round'><polyline points='20 6 9 17 4 12'/></svg></button>
            <button style="width:34px;height:34px;flex-shrink:0;border-radius:10px;border:none;background:#FEE2E2;cursor:pointer;display:inline-flex;align-items:center;justify-content:center" title="Reject" onclick="rejectLoan('${l.id}')"><svg width='14' height='14' viewBox='0 0 24 24' fill='none' stroke='#991B1B' stroke-width='2.5' stroke-linecap='round'><line x1='18' y1='6' x2='6' y2='18'/><line x1='6' y1='6' x2='18' y2='18'/></svg></button>` : ''}
          ${l.status==='Active' && canApproveFinance() ? `<button style="width:34px;height:34px;flex-shrink:0;border-radius:10px;border:none;background:#DBEAFE;color:#1E40AF;cursor:pointer;display:inline-flex;align-items:center;justify-content:center" title="Record Payment" onclick="recordLoanPayment('${l.id}')">${ICO.dollar}</button>` : ''}
          ${['Active','Rejected'].includes(l.status) && canApproveFinance() && l.amountPaid===0 ? `<button style="width:34px;height:34px;flex-shrink:0;border-radius:10px;border:none;background:var(--gray-100);color:var(--gray-600);cursor:pointer;display:inline-flex;align-items:center;justify-content:center" title="Roll back to Pending" onclick="rollbackLoan('${l.id}')">${ICO.refresh}</button>` : ''}
          </td> </tr>`;
...
function renderPayrollPreview(rows) {
  ...
  body.innerHTML = rows.map(({ emp, payrow, calc }) => {
    ...
    totNet    += calc.netPay;
    return `<tr> <td><div style="font-weight:700;font-size:12px">${emp.name}</div><div style="font-size:10px;color:var(--gray-400)">${emp.id}</div></td> <td style="font-family:var(--mono);font-size:12px">${fmtCurrency(calc.base)}</td> <td style="font-family:var(--mono);font-size:12px">${fmtCurrency(calc.allow)}</td> <td style="font-family:var(--mono);font-size:12px">${calc.otHours||0}h</td> <td style="font-family:var(--mono);font-size:12px">${fmtCurrency(calc.otPay)}</td> <td style="font-family:var(--mono);font-size:12px;color:var(--green)">${fmtCurrency(calc.perfBonus||0)}</td> <td style="font-family:var(--mono);font-size:12px;color:var(--red)">${fmtCurrency(calc.loanDeduct||0)}</td> <td style="font-family:var(--mono);font-size:12px;color:var(--amber)">${fmtCurrency(calc.advanceDeduct||0)}</td> <td style="font-family:var(--mono);font-size:12px;font-weight:700;color:var(--blue)">${fmtCurrency(calc.grossEarnings)}</td> <td style="font-family:var(--mono);font-size:12px;color:var(--red)">${fmtCurrency(calc.totalDeductions)}</td> <td style="font-family:var(--mono);font-size:13px;font-weight:900;color:var(--green)">${fmtCurrency(calc.netPay)}</td> </tr>`;
  }).join('');
...
function exportPayrollPreview() {
  ...
  const headers = ['Employee ID','Name','Subsidiary','Department','Base Salary','Allowance','OT Hours','OT Pay','Performance Bonus','Gross Earnings','Loan Deduction','Advance','Total Deductions','Net Pay'];
  const data = rows.map(({emp,calc}) => [
    ...
    calc.totalDeductions.toFixed(2), calc.netPay.toFixed(2),
  ]);
  ...
  openModal('xl', `
    <div style="display:flex;flex-direction:column;height:85vh;overflow:hidden"> <!-- ── HEADER BANNER ── --> <div style="background:linear-gradient(135deg,var(--navy),#002D72);padding:20px 24px;flex-shrink:0"> <div style="display:flex;align-items:center;gap:16px;flex-wrap:wrap"> <div style="width:64px;height:64px;border-radius:50%;background:linear-gradient(135deg,var(--gold),#E8B84B);display:flex;align-items:center;justify-content:center;font-size:22px;font-weight:900;color:var(--navy);border:3px solid rgba(255,255,255,.2);flex-shrink:0"> ${initials(e.name)}
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px"> <div class="card" style="margin:0"><div class="card-header"><span class="card-title">Current Payslip — ${STATE.payMonth||new Date().toISOString().slice(0,7)}</span></div> <div class="card-body"> <div class="section-divider"><span>Earnings</span></div> ${[['Base Salary',fmtCurrency(payCalc.base)],['Allowances',fmtCurrency(payCalc.allow)],['OT Pay ('+payCalc.otHours+'hrs)',fmtCurrency(payCalc.otPay)],['Performance Bonus',fmtCurrency(payCalc.perfBonus||0)]].map(([k,v])=>`<div class="info-row"><span class="info-key">${k}</span><span class="info-val" style="font-family:var(--mono)">${v}</span></div>`).join('')}
                <div class="info-row" style="font-weight:800;border-top:1px solid var(--border);padding-top:8px;margin-top:4px"><span class="info-key">Gross Earnings</span><span class="info-val" style="font-family:var(--mono);color:var(--blue)">${fmtCurrency(payCalc.grossEarnings)}</span></div> <div class="section-divider" style="margin-top:12px"><span>Deductions</span></div> ${[['Loan Deduction',payCalc.loanDeduct||0],['Salary Advance',payCalc.advanceDeduct||0],['Late Deduction',payCalc.lateDeduction||0]].filter(([,v])=>v>0).map(([k,v])=>`<div class="info-row"><span class="info-key">${k}</span><span class="info-val" style="font-family:var(--mono);color:var(--amber)">-${fmtCurrency(v)}</span></div>`).join('')}${payCalc.totalDeductions===0?`<div class="info-row"><span class="info-key" style="color:var(--gray-400)">No deductions</span><span class="info-val">—</span></div>`:''}
                <div class="info-row" style="font-weight:900;border-top:2px solid var(--border);padding-top:10px;margin-top:6px;font-size:16px"><span class="info-key">NET PAY</span><span class="info-val" style="font-family:var(--mono);color:var(--green);font-size:18px">${fmtCurrency(payCalc.netPay)}</span></div> </div> </div> <div class="card" style="margin:0"><div class="card-header"><span class="card-title">Salary Structure</span></div> <div class="card-body"> ${[['Monthly Gross',fmtCurrency(payCalc.grossEarnings)],['Annual Gross',fmtCurrency(payCalc.grossEarnings*12)],['Monthly Net',fmtCurrency(payCalc.netPay)],['Annual Net',fmtCurrency(payCalc.netPay*12)],['Deduction Rate',((payCalc.totalDeductions/payCalc.grossEarnings)*100).toFixed(1)+'%']].map(([k,v])=>`<div class="info-row"><span class="info-key">${k}</span><span class="info-val" style="font-family:var(--mono)">${v}</span></div>`).join('')}
